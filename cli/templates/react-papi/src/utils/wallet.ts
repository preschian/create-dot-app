...
export const walletSelectedAccount = persistentAtom('wallet:selectedAccount', '', {
  encode: (account: any) => account ? JSON.stringify(account) : '',
  decode: (str: string) => str ? JSON.parse(str) : null,
})
...
export async function connectWallet(extensionName?: string): Promise<void> {
  ...
  try {
    ...
    if (extensionName) {
      accounts = allAccounts.filter((account: InjectedAccountWithMeta) => {
...
export async function getSigner(address?: string): Promise<any> {
  ...
  try {
    ...
    const account = accounts.find((acc: any) => acc.address === targetAddress)
    if (!account) {
