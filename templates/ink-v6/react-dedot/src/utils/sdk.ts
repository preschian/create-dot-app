...
const clients: Partial<Record<Prefix, Promise<DedotClient<ApiTypeFor<Prefix>>>>> = {}

export default function sdk<T extends Prefix>(chain: T): { api: DedotApiFor<T> } {
  if (!clients[chain]) {
    clients[chain] = DedotClient.new(new WsProvider([...CHAIN_CONFIG[chain].providers]))
  }

  return {
    api: clients[chain]! as DedotApiFor<T>,
  }
}
