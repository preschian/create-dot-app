...
// Editorial first-run welcome screen for create-dot-app (dedot). Reproduces the
// shared design (layout, type, accent picker, light/dark) with a live block
// watcher, network switch, Connect Wallet, and a sample extrinsic wired to the
// template's real dedot + Talisman hooks.
...
import { LiveDemo } from './welcome/panels/LiveDemo'
import { NetworkSwitch } from './welcome/panels/NetworkSwitch'
import { WalletConnect } from './welcome/panels/WalletConnect'
import { DEFAULT_ACCENT, themeVars, tokens } from './welcome/theme'
import { Ic } from './welcome/ui/icons'

export default function App() {
  const [dark, setDark] = useState(false)
  const [accent, setAccent] = useState(DEFAULT_ACCENT)
  const [chainKey, setChainKey] = useState<Prefix>(DEFAULT_NETWORK.key)

  const net = networkByKey(chainKey)
  const C = tokens(dark)
      ...
      <div className="mx-auto min-h-screen max-w-[1280px] border-x border-(--line)">
          ...
          <div className="flex flex-col gap-3 overflow-visible sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            ...
            <div className="flex flex-col gap-2 overflow-visible sm:flex-row sm:items-center sm:gap-3">
              <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 overflow-visible">
                <NetworkSwitch net={net} onSwitch={setChainKey} />
                <WalletConnect net={net} />
              </div>
          ...
          <aside className="flex flex-col welcome-md:border-t welcome-md:border-(--line)">
            <div className="border-b border-(--line) bg-(color-mix(in_srgb,var(--acc)_10%,transparent)) p-6 px-7">
              ...
              <div className="mt-2 text-[17px] leading-snug font-semibold">
                Edit
                {' '}
                <span className="font-mono text-sm font-medium text-(--acc)">app/components/app.tsx</span>
                {' '}
                and save. It reloads instantly.
              </div>
            </div>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-(--line) px-10 py-[18px] welcome-sm:px-5">
          <span className="font-mono text-[11.5px] text-(--faint)">
            generated by
            {' '}
            <span className="text-(--dim)">npm create dot-app@latest</span>
          </span>
